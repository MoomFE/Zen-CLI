...
module.exports = {
  ...
  run( config ){
    ...
    options.assets.$each( options => {
      ...
      if( !options.gzipSize ){
        options.gzipSize = zlib.gzipSync( fs.readFileSync( options.name ) ).length;
      }

      options.sizeString = getSize( options.size );
      options.gzipSizeString = getSize( options.gzipSize )
    });

    // 执行事件回调
    if( config.on && config.on.WriteFile ){
      options.assets.$each( options => {
        config.on.WriteFile( webpackConfig, config, options.name, options.size, options.gzipSize );
      });
    }
    ...
    options.assets.$each(( options, index ) => {
      this.log(
        `${ index ? '      ' : 'Output' }  : ${ options.name.green } - ( ${ options.sizeString.green } / ${ options.gzipSizeString.green } )`,
      );
    });
